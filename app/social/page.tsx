  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const [{ data: sectionData }, { data: mediaData }] = await Promise.all([
        supabase.from("sections").select("title, description, tagline").eq("slug", "social").single(),
        supabase.from("gallery_assets").select("*").eq("category", "social").order("order_index", { ascending: true }),
      ]);
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-6 sm:px-12 pb-32">
      ...
      {loading ? (
        ...
        media.length > 0 && <MediaGrid items={media} sectionTitle={sectionInfo?.title || "Social"} />
      )}
    </div>
  );
}
